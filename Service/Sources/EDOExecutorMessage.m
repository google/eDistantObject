...
+ (instancetype)messageWithRequest:(EDOServiceRequest *)request
                           channel:(id<EDOChannel>)channel
                           service:(EDOHostService *)service {
  return [[self alloc] initWithRequest:request channel:channel service:service];
}

+ (instancetype)emptyMessage {
  return [[self alloc] initWithRequest:nil channel:nil service:nil];
}
